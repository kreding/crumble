...
(function ($, Bubble) {
	...
	var defaults = {
		...
		// position: {vertical : 'bottom', horizon: 'center'}, // top, middle, bottom |left, center, right
		vertical : 'bottom',  // top, middle, bottom
		horizon: 'center', // left, center, right
		showAfter: 0,
		hideAfter: false,
		hideOnClick: false,
		hasHideButton: false,
    ...
    $.fn.grumble = function (settings, adjustments) {
		...
		return this.each(function () {
			...
			offset = getOffsets($me, context, options.position);
	...
	function getOffsets($me, context, cusPosition){
		var offset,
			marginTop,
			cusLocation={};

		if(cusPosition.horizon === 'left' ){
			cusLocation.horizon = 0;
		}else if(cusPosition.horizon === 'right') {
			cusLocation.horizon = $me.width();
		}else {
			cusLocation.horizon = $me.width()/2;
		}

		if(cusPosition.vertical === 'top' ){
			cusLocation.vertical = 0;
		}else if(cusPosition.vertical === 'bottom') {
			cusLocation.vertical = $me.height();
		}else {
			cusLocation.vertical = $me.height()/2;
		}

		if( context ) {
			marginTop = Number($me.css('margin-top').replace("px", "")) || 0;
			offset = $me.position();
			offset.top += marginTop + context.scrollTop() + cusLocation.vertical;
			offset.left += cusLocation.horizon;
		} else {
			offset = $me.offset();
			offset.top +=  cusLocation.vertical;
			offset.left += cusLocation.horizon;
		}
